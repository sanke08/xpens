...
export default function TransactionScreen() {
  ...
  const { addTransaction, updateTransaction, transactions, categories } = useStore();
  
  const existingTx = typeof id === 'string' ? transactions.find(t => t.id === id) : undefined;
  ...
  useEffect(() => {
    if (existingTx) {
      setInputText(existingTx.amount.toString() + (existingTx.note ? ` ${existingTx.note}` : ''));
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollArea} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          <Text style={styles.headerTitle}>{existingTx ? 'Edit Transaction' : 'Add Transaction'}</Text>
        </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity 
          ...
          activeOpacity={0.8}
        >
          <Text style={styles.saveBtnText}>{existingTx ? 'Update' : 'Save'}</Text>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
